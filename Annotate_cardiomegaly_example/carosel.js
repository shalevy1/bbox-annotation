var VG = (function(vg) {
  ...
  vg.Carosel = function(container_div, button_div, num_panes,
    ...
    var next_button = $('<button>').prop('disabled', true)
                                   ...
                                   .addClass('btn btn-default btn-lg padded')
                                   .appendTo(button_div);
	var add_button = $('<button>').prop('disabled', true)
                                   .text('Add another box (up arrow)')
                                   .addClass('btn btn-default btn-lg padded')
                                   .appendTo(button_div);
    ...
    var next = function() { if (enabled) select_pane(current_idx + 1, should_scroll, 0); };
    var prev = function() { if (enabled) select_pane(current_idx - 1, should_scroll, 0); };
	var add_b = function() { if (enabled) {select_pane(current_idx + 1, should_scroll, 1); } };

    next_button.click(next);
    prev_button.click(prev);
	add_button.click(add_b);
    ...
    $(document.documentElement).keyup(function(e) {
		...
		if ($.inArray(e.keyCode, addKeys) !== -1) add_b();
		// if ($.inArray(e.keyCode, addKeys) !== -1) vg.();
	});
    ...
    function select_pane(new_idx, scroll, ifadd=0) {
	  // if (ifadd) {
		  // num_panes;
	  // {
	
      // Ignore out-of-bounds calls
      if ((new_idx < 0 || new_idx >= num_panes) && !ifadd) return;
	  ...
	  if (ifadd){
		for (var i = divs.length;i > new_idx; i--) {
				  divs[i] = divs[i-1];
		}
		divs[new_idx] = $('<div>').appendTo(container_div).hide();
	  }
      ...
      if (scroll) {
        show_callback(current_idx,new_idx,ifadd, divs[new_idx], cb);
      } else {
        show_callback(current_idx,new_idx,ifadd, divs[new_idx]);
      }
	  if (ifadd) num_panes++;
